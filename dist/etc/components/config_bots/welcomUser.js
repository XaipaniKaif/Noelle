...
export async function welcomSetting(interaction) {
...
async function quickSetup(i) {
    ...
    await createWelcomMessage(channelQuickSetting, true);
}
async function createWelcomMessage(channel, quickSetup) {
    const guideMessage = await channel.send({ embeds: [welcom.createMessageWelcomTutorial()] });
    const filter = (m) => !m.author.bot && m.attachments.size === 0 && m.stickers.size === 0 && m.content.length <= 4000;
    const collector = channel.createMessageCollector({ max: 1, time: 360_000, filter: filter });
    collector.on('collect', async (m) => {
        const content = m.content;
        const sql = `INSERT INTO ?? (id_guild, welcom_message) VALUES (?, ?) ON DUPLIKATE KEY UPDATE welcom_message = VALUES(welcom_message)`;
...
async function selectChannelWelcom(interaction, channel, quickSetup) {
    let messageReply;
    if (quickSetup === true) {
